    ...
    <div className="member-dashboard-grid">
      ...
      <section className="member-content">
        ...
        {!data.identity.acceptanceComplete && !acceptanceStepComplete ? <section className="dashboard-panel onboarding-panel acceptance-panel"><span className="dashboard-kicker">MEMBERSHIP CONFIRMATION</span><h1>Confirm your membership.</h1><p>These confirmations are recorded with your account when you continue.</p><div className="acceptance-checks"><label><input type="checkbox" checked={acceptedAdult} onChange={(event) => setAcceptedAdult(event.target.checked)}/><span>I attest that I am at least 18 and have reached the age of majority where I live.</span></label><label><input type="checkbox" checked={acceptedTerms} onChange={(event) => setAcceptedTerms(event.target.checked)}/><span>I have read and accept the <a href="/terms" target="_blank">Terms</a>.</span></label><label><input type="checkbox" checked={acceptedPrivacy} onChange={(event) => setAcceptedPrivacy(event.target.checked)}/><span>I have read and accept the <a href="/privacy" target="_blank">Privacy Policy</a>.</span></label></div><button disabled={busy || !acceptanceReady} onClick={data.account.type ? acceptPolicies : () => setAcceptanceStepComplete(true)}>{busy ? "Saving…" : "Accept & continue"}</button></section> : !data.account.type ? <section className="dashboard-panel onboarding-panel"><span className="dashboard-kicker">WELCOME</span><h1>Choose how you will use Crypto Sugar.</h1><div className="onboarding-grid"><article><h2>Sugar Babe</h2><p>Submit a public profile, add up to eight photos free, and message free using email. Profiles and photos are reviewed regularly by administrators.</p><button disabled={busy} onClick={chooseCreator}>Continue as creator</button></article><article><h2>Sugar Daddy</h2><p>Customers stay private, message freely, and connect a wallet only for paid support. Add your display name and bio later in your dashboard.</p><button disabled={busy} onClick={() => chooseCustomer()}>Continue as customer</button></article></div></section> : <>
          ...
          {tab === "profile" && <section>
            <div className="dashboard-heading"><span className="dashboard-kicker">{creator ? "CREATOR PROFILE" : "PRIVATE PROFILE"}</span><h1>{creator ? "Your public presence" : "Your private account"}</h1><p>{creator ? "Submit profile changes and new photos here. Profiles and photos are reviewed regularly by administrators." : "Only creators you message can see your display name and private introduction."}</p></div>
            {creator ? <>
              {!data.identity.walletAddress && <div className="dashboard-warning"><strong>Connect a payout wallet to receive paid support</strong><p>{data.paymentCapabilities.baseCreatorSupportEnabled ? "Paid likes, gifts, and boosts support USDC on Solana and Base." : "Paid likes, gifts, and boosts currently use Solana USDC. Base wallets can be linked for sign-in until atomic Base settlement is activated."}</p><div className="payout-wallet-actions"><a href="/?connectPayout=1&network=solana">Link Solana payout wallet</a><a href="/?connectPayout=1&network=evm">{data.paymentCapabilities.baseCreatorSupportEnabled ? "Link Base payout wallet" : "Link Base for sign-in only"}</a></div></div>}
              {data.identity.walletChain === "evm" && !data.paymentCapabilities.creatorSupportEnabled && <div className="dashboard-warning"><strong>Base wallet linked for sign-in</strong><p>Paid likes, gifts, and boosts remain disabled until atomic Base settlement is deployed. Link Solana now to receive USDC support.</p><div className="payout-wallet-actions"><a href="/?connectPayout=1&network=solana">Link Solana payout wallet</a></div></div>}
              <form className="dashboard-panel dashboard-form" onSubmit={saveCreator}>
                ...
                {managedPhotos.length ? <div className="dashboard-current-photos"><div className="current-photo-heading"><div><strong>Current photos</strong><p>Drag anywhere on a photo card to reorder. Use the strip over the image to reposition its framing. The first image is your display photo.</p></div>{photoLayoutSaving ? <span>Saving arrangement…</span> : <span>Changes save automatically</span>}</div><div className="dashboard-photo-grid">{managedPhotos.map((photo, index) => <div className={`photo-editor-card ${draggedPhotoId === photo.id ? "dragging" : ""}`} key={photo.id} draggable={!busy && !photoLayoutSaving} onDragStart={(event) => beginPhotoDrag(event, photo.id)} onDragEnd={() => setDraggedPhotoId("")} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={(event) => dropPhoto(event, photo.id)} aria-label={`Drag photo ${index + 1} to reorder`}><div className="photo-card-toolbar"><button className="photo-drag-handle" type="button">↕ Drag to reorder</button>{index === 0 ? <span>Display photo</span> : <button type="button" disabled={busy || photoLayoutSaving} onClick={() => movePhoto(photo.id, -1)}>Move earlier</button>}</div><div className="photo-editor-frame"><img draggable={false} src={photo.url} alt={`Current profile photo ${index + 1}`} style={{ objectPosition: `${photo.focalX}% ${photo.focalY}%` }}/><span className="reposition-hint" onPointerDown={(event) => beginReposition(event, "current", photo.id, photo.focalX, photo.focalY)} onPointerMove={repositionPhoto} onPointerUp={finishReposition} onPointerCancel={finishReposition}>Drag here to reposition</span></div><small>{photo.approved ? "Published" : "Hidden"} · {photo.paidLikes} likes</small><div className="photo-card-actions"><button type="button" disabled={busy || photoLayoutSaving || index === 0} onClick={() => movePhoto(photo.id, -1)} aria-label="Move photo left">←</button><button type="button" disabled={busy || photoLayoutSaving || index === managedPhotos.length - 1} onClick={() => movePhoto(photo.id, 1)} aria-label="Move photo right">→</button><button type="button" disabled={busy || photoLayoutSaving} onClick={() => removePhoto(photo.id)}>Remove</button></div></div>)}</div></div> : null}
            ...
            </> : <><form className="dashboard-panel dashboard-form" onSubmit={saveCustomer}><label>Private display name<input required maxLength={80} value={customerName} onChange={(event) => setCustomerName(event.target.value)}/></label><label>Private introduction<textarea maxLength={300} value={customerBio} onChange={(event) => setCustomerBio(event.target.value)}/></label><button disabled={busy}>Save private profile</button></form><div className="dashboard-panel"><div className="dashboard-panel-title"><h2>Favorites</h2><span>{data.favorites.length}</span></div><div className="favorite-dashboard-grid">{data.favorites.map((item) => <a href={`/?profile=${item.id}`} key={item.id}>{item.imageUrl ? <img src={item.imageUrl} alt=""/> : <span>{item.name[0]}</span>}<strong>{item.name}</strong><small>{item.country} · {item.region}</small></a>)}{!data.favorites.length && <p className="dashboard-empty">No saved creators yet.</p>}</div></div></>}
          </section>}
