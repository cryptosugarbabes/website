...
export function MemberDashboard() {
  ...
  async function chooseCreator() {
    ...
    if (!response.ok) setError(next.error || "Could not create your creator account.");
      ...
      setNotice("Your creator account is ready. Add your profile and up to four photos.");
      await loadDashboard();
      setTab("profile");
    }
    setBusy(false);
  ...
  async function saveCreator(event: FormEvent<HTMLFormElement>) {
    ...
    if (existingPhotos + files.length > 4) {
      ...
      setError(`A creator profile can have up to four photos. Remove ${excess} photo${excess === 1 ? "" : "s"} before uploading these files.`);
      setBusy(false);
      return;
    }
