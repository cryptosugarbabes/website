...
type ConversationMessage = { id: string; body: string; mine: boolean; status: string; automated?: boolean; automationMatched?: boolean | null; boostAmountUsdc?: number; boostedAt?: string | null; createdAt: string };
  ...
  return (
    <main>
      ...
      <section className="crypto-section">
        <div className="crypto-copy"><span className="section-kicker">USDC-READY EARNINGS</span><h2>Your wallet unlocks earnings.<br/>Not membership.</h2><p>Creators can join by email, publish up to eight photos free, and message for free. Connect Base or Solana only when you are ready to receive paid likes, gifts, and boosts.</p><ul><li><Icon name="check" size={16}/>MetaMask, Binance, Trust, Rabby, and Coinbase on Base</li><li><Icon name="check" size={16}/>Solflare and Phantom on Solana</li><li><Icon name="check" size={16}/>Profiles and messages work without a wallet</li><li><Icon name="check" size={16}/>A verified wallet is required before receiving USDC</li></ul></div>
      ...
      <section className="safety-section" id="safety"><div className="safety-mark"><Icon name="shield" size={31}/></div><div className="safety-copy"><span className="section-kicker">SAFETY IS THE PRODUCT</span><h2>Adults only. Consent always.</h2><a href="mailto:email@cryptosugarbabes.com?subject=Safety%20report">Contact safety <Icon name="arrow" size={17}/></a></div><p>Crypto Sugar is designed for lawful social discovery and companionship. Coercion, trafficking, underage users, non-consensual content, extortion, and unlawful activity are prohibited and subject to removal.</p></section>

      <footer><div className="brand-social"><a className="brand" href="#top"><img className="brand-logo-image" src="/csb-coin-logo.png" alt=""/><span>Crypto Sugar Babes</span></a><InstagramLink/><XLink/></div><p className="footer-copyright">© 2026 Crypto Sugar Babes. Safety First Always.</p><div className="footer-links"><a href="/safety">Safety</a><a href="/disputes">Disputes</a><a href="/terms">Terms</a><a href="/privacy">Privacy</a></div></footer>
        ...
        return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setActiveProfile(null); }}>
          <section className="profile-modal" role="dialog" aria-modal="true" aria-labelledby="profile-modal-title">
            ...
            <div className="modal-content">
              ...
              <div className="modal-actions"><button className="primary-button message-send-button" onClick={() => openMessage(activeProfile)}><Icon name="message" size={18}/>Send message</button><button className="gift-action" disabled={paymentBusy || !activeProfile.supportEnabled} onClick={() => openGift(activeProfile)} title={activeProfile.supportEnabled ? "Send a USDC gift" : "This creator has not connected a payout wallet yet"}><Icon name="spark" size={16}/>{activeProfile.supportEnabled ? "Gift" : "Gifts not enabled"}</button><button className={`heart-action ${favorites.has(activeProfile.id) ? "active" : ""}`} onClick={() => toggleFavorite(activeProfile)}><Icon name="heart" size={19} filled={favorites.has(activeProfile.id)}/></button></div>
              ...
              <p className="modal-footnote">Messages are delivered free. {activeProfile.supportEnabled ? "Paid likes, gifts, and boosts use USDC and are split 90% to the creator and 10% to the platform after on-chain confirmation." : "Paid support becomes available after the creator connects a verified payout wallet."}</p>
            </div>
          </section>
        </div>;
      })()}
      ...
      {profileOpen && <div className="modal-backdrop profile-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setProfileOpen(false); }}><section className="create-modal" role="dialog" aria-modal="true" aria-labelledby="create-title"><button className="modal-close" onClick={() => setProfileOpen(false)} aria-label="Close profile creator"><Icon name="close" size={20}/></button><div className="create-heading"><h2 id="create-title">Create your profile.</h2><p>Preview your free profile now. It publishes automatically and remains subject to administrator review.</p></div><form onSubmit={submitProfile}>
        ...
        {walletError && !walletPickerOpen && <div className="form-error">{walletError}</div>}<div className="form-footer"><p>{wallet ? `Earnings enabled with ${walletName || walletChain}` : "Email access is enough to publish; connect a wallet later to earn"}</p><button className="primary-button profile-publish-button" type="submit" disabled={profileSaving}>{profileSaving ? `Saving${profileFiles.length ? " & uploading…" : "…"}` : "Save & Publish"}<Icon name="arrow" size={18}/></button></div>
      </form></section></div>}
