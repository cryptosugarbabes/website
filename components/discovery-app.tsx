...
export function DiscoveryApp() {
  ...
  function handlePhotos(fileList?: FileList | null) {
    ...
    if (profilePhotos.length + files.length > 4) {
      setWalletError("You can add up to 4 photos to a profile.");
      return;
    }
    ...
    <main>
      ...
      <section className="hero" id="top">
        <div className="hero-copy-block">
          ...
          <div className="trust-row"><span><Icon name="shield" size={17}/>Adults only</span><span><Icon name="lock" size={17}/>Wallet-secured</span><span><Icon name="globe" size={17}/>Global access</span></div>
        </div>
      </section>
      ...
      <footer><div className="brand-social"><a className="brand" href="#top"><img className="brand-logo-image" src="/csb-coin-logo.png" alt=""/><span>Crypto Sugar Babes</span></a><InstagramLink/></div><p>© 2026 Crypto Sugar Babes. Adults only.</p><div className="footer-links"><a href="/safety">Safety</a><a href="/disputes">Disputes</a><a href="/terms">Terms</a><a href="/privacy">Privacy</a></div></footer>
        ...
        return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setActiveProfile(null); }}>
          <section className="profile-modal" role="dialog" aria-modal="true" aria-labelledby="profile-modal-title">
            ...
            <div className="modal-content">
              ...
              <div className="modal-actions"><button className="primary-button" onClick={() => openMessage(activeProfile)}><Icon name="message" size={17}/>Message · Free</button><button className="gift-action" disabled={paymentBusy || !activeProfile.supportEnabled} onClick={() => openGift(activeProfile)} title={activeProfile.supportEnabled ? "Send a USDC gift" : "This creator has not connected a payout wallet yet"}><Icon name="spark" size={16}/>{activeProfile.supportEnabled ? "Gift" : "Gifts not enabled"}</button><button className={`heart-action ${favorites.has(activeProfile.id) ? "active" : ""}`} onClick={() => toggleFavorite(activeProfile)}><Icon name="heart" size={19} filled={favorites.has(activeProfile.id)}/></button></div>
      ...
      {profileOpen && <div className="modal-backdrop profile-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setProfileOpen(false); }}><section className="create-modal" role="dialog" aria-modal="true" aria-labelledby="create-title"><button className="modal-close" onClick={() => setProfileOpen(false)} aria-label="Close profile creator"><Icon name="close" size={20}/></button><div className="create-heading"><span className="section-kicker">YOUR PRIVATE INTRODUCTION</span><h2 id="create-title">Create your profile.</h2><p>Preview a free draft now. Public profiles require review and adult attestation before discovery.</p></div><form onSubmit={submitProfile}>
        <div className="photo-field"><label className={profilePhotos.length ? "has-photo" : ""}>{profilePhotos.length ? <div className="photo-preview-grid">{profilePhotos.slice(0, 4).map((photo, index) => <img src={photo} alt={`Profile preview ${index + 1}`} key={`${photo.slice(-24)}-${index}`}/>)}</div> : <span><Icon name="camera" size={28}/><strong>Add up to 4 photos</strong><small>JPG, PNG or WebP · 5 MB each</small></span>}<input type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={profileSaving} onChange={(event) => handlePhotos(event.target.files)}/></label><div><strong>Your photo collection</strong><p>Photos are optimized, stripped of location metadata, and kept private until approval.</p><small>{profilePhotos.length}/4 photos selected</small>{profilePhotos.length > 0 && <button type="button" disabled={profileSaving} onClick={() => { setProfilePhotos([]); setProfileFiles([]); }}>Remove all</button>}</div></div>
