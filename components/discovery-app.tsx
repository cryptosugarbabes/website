...
export function DiscoveryApp() {
  ...
  function showWalletPicker(_context: "general" | "profile" = "general", network: WalletChain | null = null, linking = false) {
    setWalletContext(_context);
    setWalletNetworkFilter(network);
    setWalletLinking(linking);
  ...
  useEffect(() => {
    fetch("/api/auth/session").then((response) => response.json()).then(async (data: { authenticated: boolean; email: string | null; address: string | null; chain: WalletChain | null }) => {
      ...
      if (params.get("connectPayout") === "1" && data.authenticated) {
        const requestedNetwork = params.get("network") === "solana" ? "solana" : params.get("network") === "evm" ? "evm" : null;
        showWalletPicker("general", requestedNetwork, true);
        setNotice("Your current account will stay signed in while you add this wallet.");
      }
    }).catch(() => undefined);
  }, []);
  ...
  async function verifySignIn(address: string, chain: WalletChain, message: string, signature: string, name: string) {
    ...
    const params = new URLSearchParams(window.location.search);
    const linkingWallet = params.has("connectPayout");
    const returnToDashboard = linkingWallet && params.get("returnTo") === "dashboard";
    if (linkingWallet) {
    ...
    setNotice(account?.type === "CREATOR"
      ? chain === "solana"
        ? `${name} linked. Paid likes, gifts, and boosts are now enabled for your Sugar Babe profile.`
        : basePaymentsLive
    ...
    if (profileIntentRef.current && account?.type === "CREATOR") {
      ...
      profileIntentRef.current = false;
      setNotice("Sugar Daddy accounts stay private and cannot publish Sugar Babe profiles.");
    }
  }
  ...
  async function toggleFavorite(profile: Profile) {
    if (profile.sample) {
      setNotice("Editorial samples cannot be saved. Try this with a published Sugar Babe profile.");
      return;
    }
    if (!isAuthenticated) {
  ...
  async function saveAccount(type: AccountType) {
    ...
    try {
      ...
      profileIntentRef.current = false;
      setNotice(type === "CREATOR" ? "Sugar Babe account ready. Build your profile now and connect Solana when you want to earn." : "Private Sugar Daddy account ready.");
  ...
  function openMessage(profile: Profile) {
    ...
    if (profile.sample) { setNotice("Editorial samples cannot receive messages. Try a published Sugar Babe profile."); return; }
    if (!accountType) { setAccountOpen(true); return; }
    if (accountType !== "CUSTOMER") { setNotice("Sugar Babe accounts reply from their inbox; they cannot initiate Sugar Daddy conversations."); return; }
    setMessageText("");
  ...
  async function likeFeaturedPhoto(profile: Profile, requestedMediaId?: string) {
    if (!profile.supportEnabled) { setNotice("This Sugar Babe has not connected a payout wallet yet. Paid likes are unavailable."); return; }
  ...
  function openGift(profile: Profile) {
    if (!profile.supportEnabled) {
      setNotice("This Sugar Babe has not connected a payout wallet yet. Gifts are unavailable.");
      return;
    }
    if (!wallet) {
    ...
    <main>
      ...
      <header className="site-header">
        ...
        <div className="header-actions">
          ...
          {isAuthenticated ? <button className="wallet-button connected" onClick={signOut} title="Click to sign out"><span className="online-dot"/>{wallet ? `${accountType === "CREATOR" ? "Sugar Babe" : accountType === "CUSTOMER" ? "Sugar Daddy" : walletName || (walletChain === "solana" ? "Solana" : "Base")} · ${shortAddress(wallet)}` : email}</button>
      ...
      <section className="hero" id="top">
        <div className="hero-copy-block">
          ...
          <div className="hero-actions"><button className="primary-button" onClick={accountType === "CUSTOMER" && isAuthenticated ? () => document.querySelector("#discover")?.scrollIntoView({ behavior: "smooth" }) : openProfileCreator}>{accountType === "CUSTOMER" && isAuthenticated ? "Discover the Sugar" : "Be a Sugar Babe"} <Icon name="arrow" size={18}/></button>{!hasCreatorProfile && accountType !== "CUSTOMER" && <button className="secondary-button" onClick={openProfileCreator}>Be a Sugar Daddy</button>}</div>
        ...
        <div className="filter-bar">
          ...
          <div className={`select-field custom-region-select ${regionMenuOpen ? "open" : ""}`} ref={regionMenuRef}>
            ...
            <button
              ...
              type="button"
              aria-label="Filter Sugar Babes by region"
      ...
      <footer><div className="brand-social"><a className="brand" href="#top"><img className="brand-logo-image" src="/csb-coin-logo.png" alt=""/><span>Crypto Sugar Babes</span></a><InstagramLink/><XLink/></div><p className="footer-copyright">© 2026 Crypto Sugar Babes. Safety First Always.</p><div className="footer-links"><div className="footer-link-row"><a href="/disputes">Disputes</a><a href="/safety">Safety</a><a href="/terms">Terms</a><a href="/privacy">Privacy</a></div><div className="footer-link-row"><a href="/anti-slavery">Anti-Slavery</a><a href="/anti-trafficking">Anti-Trafficking</a><a href="/consumer-protection">Consumer Protection</a></div></div></footer>
        ...
        return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setActiveProfile(null); }}>
          <section className="profile-modal" role="dialog" aria-modal="true" aria-labelledby="profile-modal-title">
            ...
            <div className="modal-content">
              ...
              <p className="modal-footnote">Messages are delivered free. {activeProfile.supportEnabled ? "Paid likes, gifts, and boosts use USDC and are split 90% to the Sugar Babe and 10% to the platform after on-chain confirmation." : "Paid support becomes available after the Sugar Babe connects Solana, or when atomic Base settlement is enabled."}</p>
            </div>
          </section>
        </div>;
      })()}
      ...
      {messageTarget && (() => {
        ...
        return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setMessageTarget(null); }}>
          <section className="message-modal" role="dialog" aria-modal="true" aria-labelledby="message-title">
            ...
            <p className="wallet-intro">Every normal message is free. You may optionally attach a boost so your introduction is highlighted and your monthly Sugar Daddy rank grows.</p>
            <form onSubmit={sendMessage}><label className="message-field"><span>YOUR MESSAGE</span><textarea required maxLength={800} autoFocus value={messageText} onChange={(event) => setMessageText(event.target.value)} placeholder="Start with something thoughtful…"/><small>{messageText.length}/800</small></label>
              <div className="boost-panel"><div><span>OPTIONAL MESSAGE BOOST</span><small>{messageTarget.supportEnabled ? "Boosted introductions receive priority placement." : "This Sugar Babe has not enabled USDC earnings yet. Your message remains free."}</small></div><div className="preset-buttons"><button type="button" className={!messageBoostAmount ? "active" : ""} onClick={() => setMessageBoostAmount("")}>Free</button>{messageTarget.supportEnabled && [5, 10, 25].map((amount) => <button type="button" className={messageBoostAmount === String(amount) ? "active" : ""} onClick={() => setMessageBoostAmount(String(amount))} key={amount}>{amount} USDC</button>)}</div>{messageTarget.supportEnabled && <label><span>Custom boost</span><input type="number" min="0" max="100000" step="0.01" value={messageBoostAmount} onChange={(event) => setMessageBoostAmount(event.target.value)} placeholder="0.00"/></label>}</div>
              <div className="message-checkout"><div><span>Normal message</span><strong>Free</strong></div><div><span>Sugar Babe support score</span><strong>{stats.points} points</strong></div><div className="message-total"><span>{messageBoostAmount ? "Optional boost" : "Amount due"}</span><strong>{messageBoostAmount ? `${formatUsdc(Number(messageBoostAmount) || 0)} USDC` : "Free"}</strong></div></div>
      ...
      {giftTarget && (() => {
        ...
        return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setGiftTarget(null); }}>
          <section className="message-modal gift-modal" role="dialog" aria-modal="true" aria-labelledby="gift-title">
            ...
            <p className="wallet-intro">Choose any amount. Gifts increase {giftTarget.name}&apos;s support score and count toward your monthly Sugar Daddy rank.</p>
            <form onSubmit={sendGift}><div className="preset-buttons gift-presets">{[5, 25, 50, 100].map((preset) => <button type="button" className={giftAmount === String(preset) ? "active" : ""} onClick={() => setGiftAmount(String(preset))} key={preset}>{preset} USDC</button>)}</div><label className="gift-amount"><span>CUSTOM GIFT</span><input type="number" min="1" max="100000" step="0.01" required value={giftAmount} onChange={(event) => setGiftAmount(event.target.value)} placeholder="25.00"/></label>
              <div className="message-checkout"><div><span>Sugar Babe receives 90%</span><strong>{formatUsdc(creatorShareUsdc(amount))} USDC</strong></div><div><span>Platform receives 10%</span><strong>{formatUsdc(platformShareUsdc(amount))} USDC</strong></div><div className="message-total"><span>Your Sugar Rating after gift</span><strong>{formatMonthlySugarRating(monthlySupportGivenUsdc + amount)} · {sugarDaddyMonthlyLevel(monthlySupportGivenUsdc + amount).name}</strong></div></div>
