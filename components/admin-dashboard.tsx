...
type VisitorChatSession = { id: string; shortId: string; status: string; pagePath: string; email?: string | null; archivedAt?: string | null; loggedIn: boolean; memberName?: string | null; memberEmail?: string | null; memberAccountType?: string | null; messageCount: number; visitorMessageCount: number; createdAt: string; lastSeenAt: string };
...
type VisitorChatView = "inbox" | "archive";
...
function label(value: string) {
  if (value === "CREATOR") return "Sugar Babe";
  if (value === "CUSTOMER") return "Sugar Daddy";
  return value.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase());
}
function adminTabLabel(value: AdminTab) { return value === "visitor-chat" ? "Pop-up Chat" : label(value); }
...
export function AdminDashboard() {
  ...
  const [visitorChatView, setVisitorChatView] = useState<VisitorChatView>("inbox");
  const [visitorChatCounts, setVisitorChatCounts] = useState({ inbox: 0, archive: 0 });
  ...
  async function loadVisitorChats(view: VisitorChatView = visitorChatView) {
    const response = await fetch(`/api/admin/visitor-chats?view=${view}`, { cache: "no-store" });
    if (response.status === 401) { setSignedIn(false); return; }
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Could not load pop-up chats.");
    setVisitorChats(data.sessions || []);
    setVisitorChatCounts(data.counts || { inbox: 0, archive: 0 });
  }
  ...
  useEffect(() => {
    if (!signedIn || tab !== "visitor-chat") return;
    const timer = window.setInterval(() => loadVisitorChats(visitorChatView).catch(() => undefined), 5_000);
    return () => window.clearInterval(timer);
  }, [signedIn, tab, visitorChatView]);
  ...
  async function review(id: string, action: "approve" | "reject") {
    const reason = action === "reject" ? window.prompt("Reason shown to the Sugar Babe:")?.trim() : "";
  ...
  async function deleteProfile(profile: ReviewProfile) {
    const confirmation = window.prompt(`Delete ${profile.name}'s public profile and photos? The member account and required transaction records will remain. Type DELETE to confirm:`)?.trim();
    if (confirmation !== "DELETE") return;
    const reason = window.prompt("Reason shown to the Sugar Babe:")?.trim() || "";
    ...
    const data = await response.json();
    if (!response.ok) setError(data.error || "Profile deletion failed.");
    else await Promise.all([loadProfiles(), loadOperations()]);
    setBusy("");
  }
  async function reviewPhoto(photoId: string, action: "approve" | "reject") {
    const reason = action === "reject" ? window.prompt("Reason shown to the Sugar Babe for this photo:")?.trim() : "";
    if (action === "reject" && !reason) return;
  ...
  async function openVisitorChat(session: VisitorChatSession) {
    ...
    const data = await response.json();
    if (!response.ok) setError(data.error || "Pop-up chat access failed.");
    else { setVisitorTranscript(data); setVisitorReply(""); }
    setBusy("");
  }

  async function selectVisitorChatView(view: VisitorChatView) {
    setVisitorChatView(view);
    setVisitorTranscript(null);
    setError("");
    await loadVisitorChats(view).catch((caught) => setError(caught.message));
  }

  async function moveVisitorChat(session: VisitorChatSession, action: "ARCHIVE" | "RESTORE") {
    const busyId = `visitor-${action.toLowerCase()}-${session.id}`;
    setBusy(busyId); setError("");
    const response = await fetch("/api/admin/visitor-chats", {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    if (!response.ok) setError(data.error || "Pop-up chat could not be moved.");
    else {
      if (visitorTranscript?.session.id === session.id) setVisitorTranscript(null);
      await loadVisitorChats(visitorChatView);
    }
    setBusy("");
  }
  ...
  return <main className="admin-shell admin-console">
    ...
    <div className="admin-console-grid"><aside><nav>{(["overview", "profiles", "accounts", "visitor-chat", "conversations", "forums", "notifications", "payments", "reports", "audit"] as AdminTab[]).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{adminTabLabel(item)}{item === "profiles" && pending.length ? <em>{pending.length}</em> : null}{item === "visitor-chat" && visitorChatCounts.inbox ? <em>{visitorChatCounts.inbox > 99 ? "99+" : visitorChatCounts.inbox}</em> : null}{item === "reports" && activeReports.length ? <em>{activeReports.length}</em> : null}</button>)}</nav></aside><section className="admin-console-content">{error && <div className="form-error">{error}</div>}
      {tab === "overview" && <><div className="admin-page-heading"><span>LIVE OPERATIONS</span><h2>Platform overview</h2><p>Membership, moderation and confirmed settlement at a glance.</p></div><div className="admin-metric-grid"><article><span>Accounts</span><strong>{metrics.accounts}</strong><small>{metrics.creators} Sugar Babes · {metrics.customers} Sugar Daddies</small></article><article><span>Needs review</span><strong>{pending.length}</strong><small>{metrics.openReports} open safety reports</small></article><article><span>Confirmed volume</span><strong>{money(metrics.grossUsdc)}</strong><small>{metrics.confirmedPayments} payments</small></article><article><span>Platform revenue</span><strong>{money(metrics.platformUsdc)}</strong><small>{money(metrics.creatorUsdc)} to Sugar Babes</small></article></div>{metrics.deletionRequests > 0 && <button className="admin-alert-card" onClick={() => setTab("accounts")}><strong>{metrics.deletionRequests} deletion request{metrics.deletionRequests === 1 ? "" : "s"}</strong><span>Review account and retention obligations →</span></button>}<div className="admin-section-heading admin-funnel-heading"><h3>{funnel.days}-day Sugar Daddy funnel</h3><span>Counts only · no IP, device or message-content tracking</span></div><div className="admin-metric-grid admin-funnel-grid"><article><span>Page views</span><strong>{funnel.pageViews}</strong><small>{funnel.signInOpens} sign-in opens</small></article><article><span>Email access</span><strong>{funnel.emailCodeRequests}</strong><small>{funnel.emailVerifications} verified users</small></article><article><span>Roles chosen</span><strong>{funnel.rolesChosen}</strong><small>{funnel.profilesSubmitted} Sugar Babe profiles</small></article><article><span>Conversations</span><strong>{funnel.conversationsStarted}</strong><small>{funnel.messagesSent} messages sent</small></article><article><span>Payment attempts</span><strong>{funnel.paymentsStarted}</strong><small>{funnel.paymentsConfirmed} confirmed</small></article></div><div className="admin-overview-columns"><article><div className="admin-section-heading"><h3>Live profiles needing review</h3><button onClick={() => setTab("profiles")}>Open profiles</button></div>{pending.slice(0, 5).map((profile) => <div className="admin-compact-row" key={profile.id}><strong>{profile.name}</strong><span>{profile.country} · {profile.region}</span><em>{profile.photos.length} photos</em></div>)}{!pending.length && <p className="admin-empty">Everything reviewed.</p>}</article><article><div className="admin-section-heading"><h3>Active safety cases</h3><button onClick={() => setTab("reports")}>Open reports</button></div>{activeReports.slice(0, 5).map((report) => <div className="admin-compact-row" key={report.id}><strong>{label(report.category)}</strong><span>{report.profileName || "Account report"}</span><em>{label(report.status)}</em></div>)}{!activeReports.length && <p className="admin-empty">No active cases.</p>}</article></div><section className="admin-error-summary"><div className="admin-section-heading"><h3>Application errors · last 30 days</h3><span>{recentErrors.reduce((total, item) => total + item.occurrences, 0)} occurrences</span></div><div className="admin-audit-list">{recentErrors.slice(0, 10).map((item) => <article key={item.id}><span>{item.occurrences}×</span><strong>{item.scope}</strong><p>{item.message}</p><small>Last seen {new Date(item.lastSeenAt).toLocaleString()}</small></article>)}{!recentErrors.length && <p className="admin-empty">No application errors recorded.</p>}</div></section></>}
      {tab === "profiles" && <><div className="admin-page-heading"><span>CONTENT REVIEW</span><h2>Sugar Babe profiles</h2><p>Profiles and photos publish automatically. Review them here at any time; suspending content removes it from public discovery, while deletion also removes its public details and photos.</p></div><section className="review-list">{profiles.length === 0 ? <p className="admin-empty">No profiles submitted.</p> : profiles.map((profile) => { const photosReady = profile.photos.length > 0 && profile.photos.every((photo) => photo.approved); return <article className="review-card" key={profile.id}><div className="review-photos">{profile.photos.length ? profile.photos.map((photo, index) => <div className="review-photo" key={photo.id}><img src={photo.url} alt={`${profile.name} photo ${index + 1}`}/><span className={photo.approved ? "approved" : "pending"}>{photo.approved ? (photo.reviewed ? "Reviewed · published" : "Live · needs review") : "Hidden"}</span><div><button disabled={busy === photo.id || (photo.approved && photo.reviewed)} onClick={() => reviewPhoto(photo.id, "approve")}>{photo.approved ? "Mark reviewed" : "Restore"}</button><button className="reject" disabled={busy === photo.id} onClick={() => reviewPhoto(photo.id, "reject")}>Reject</button></div></div>) : <div>No photos</div>}</div><div className="review-copy"><span className={`review-status status-${profile.status.toLowerCase()}`}>{profile.status === "APPROVED" ? (profile.reviewed ? "Published · reviewed" : "Live · needs review") : label(profile.status)}</span><h2>{profile.name}, {profile.age}</h2><p className="location">{profile.country} · {profile.region}</p><h3>{profile.headline}</h3><p>{profile.bio}</p><div className="tag-row">{profile.interests.map((interest) => <span key={interest}>{interest}</span>)}</div>{profile.rejectionReason && <p className="rejection-note">Previous reason: {profile.rejectionReason}</p>}{!photosReady && <p className="review-guidance">Restore every hidden photo before restoring this profile to public discovery.</p>}<div className="review-actions"><button disabled={busy === profile.id || !photosReady || profile.reviewed} onClick={() => review(profile.id, "approve")}>{profile.status === "APPROVED" ? "Mark profile reviewed" : "Restore profile"}</button><button className="reject" disabled={busy === profile.id} onClick={() => review(profile.id, "reject")}>Suspend profile</button><button className="danger-button" disabled={busy === profile.id} onClick={() => deleteProfile(profile)}>Delete profile</button></div></div></article>; })}</section></>}
      {tab === "accounts" && <><div className="admin-page-heading"><span>MEMBER OPERATIONS</span><h2>Accounts</h2><p>Search identities, review activity, manage suspensions and process deletion requests.</p></div><div className="admin-account-tools"><input className="admin-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email, wallet, role or status"/><a className="admin-export-button" href="/api/admin/members/export">Download member emails (.csv)</a></div><p className="admin-member-summary">{accounts.length} members · {memberEmailCount} verified email addresses</p><div className="admin-account-list">{filteredAccounts.map((account) => <article className={account.status === "SUSPENDED" ? "suspended" : ""} key={account.id}><header><div><strong>{account.displayName || "Unnamed member"}</strong><span>{account.type ? label(account.type) : "Role not chosen"}</span></div><em className={`review-status status-${account.status.toLowerCase()}`}>{label(account.status)}</em></header><div className="admin-account-identifiers"><span>{account.email || "No email"}</span><span>{account.walletChain ? `${label(account.walletChain)} · ${short(account.walletAddress)}` : "No wallet"}</span><span>Joined {new Date(account.createdAt).toLocaleDateString()}</span></div><div className="admin-account-stats"><span><strong>{account.conversations}</strong> conversations</span><span><strong>{account.messages}</strong> messages</span><span><strong>{money(account.supportSentUsdc)}</strong> sent</span><span><strong>{money(account.creatorEarnedUsdc)}</strong> earned</span></div>{account.profileStatus && <p>Sugar Babe profile: <strong>{label(account.profileStatus)}</strong></p>}{account.suspensionReason && <p className="rejection-note">Suspension: {account.suspensionReason}</p>}{account.deletionRequestedAt && <div className="deletion-request"><strong>Deletion requested {new Date(account.deletionRequestedAt).toLocaleDateString()}</strong><p>Complete required retention, dispute and anonymisation checks before clearing this request.</p><button disabled={busy === account.id} onClick={() => accountAction(account, "CLEAR_DELETION_REQUEST")}>Mark administratively reviewed</button></div>}<div className="review-actions">{account.status === "ACTIVE" ? <button className="reject" disabled={busy === account.id} onClick={() => accountAction(account, "SUSPEND")}>Suspend account</button> : <button disabled={busy === account.id} onClick={() => accountAction(account, "RESTORE")}>Restore account</button>}</div></article>)}</div></>}
      {tab === "visitor-chat" && <>
        <div className="admin-page-heading admin-visitor-chat-heading">
          <span>LIVE WEBSITE SUPPORT</span>
          <div><h2>Pop-up Chat</h2><nav aria-label="Pop-up chat folders"><button className={visitorChatView === "inbox" ? "active" : ""} onClick={() => selectVisitorChatView("inbox")}>Inbox <em>{visitorChatCounts.inbox}</em></button><button className={visitorChatView === "archive" ? "active" : ""} onClick={() => selectVisitorChatView("archive")}>Open archives <em>{visitorChatCounts.archive}</em></button></nav></div>
          <p>Guests and logged-in members can message here. Account status and available contact details appear on each conversation.</p>
        </div>
        <div className="admin-conversation-layout"><div className="admin-conversation-list">{visitorChats.map((session) => <article key={session.id}>
          <header><div><strong>{session.loggedIn ? session.memberName || "Logged-in member" : `Guest ${session.shortId}`}</strong><span>{session.pagePath}</span></div><small>{new Date(session.lastSeenAt).toLocaleString()}</small></header>
          {session.memberEmail && <a className="admin-visitor-email" href={`mailto:${session.memberEmail}`}>{session.memberEmail}</a>}
          {session.email && session.email !== session.memberEmail && <a className="admin-visitor-email" href={`mailto:${session.email}`}>{session.email}</a>}
          <div><span className={session.loggedIn ? "logged-in" : "guest"}>{session.loggedIn ? "Logged in" : "Guest"}</span>{session.memberAccountType && <span>{label(session.memberAccountType)}</span>}<span>{session.messageCount} messages</span><span>{session.visitorMessageCount} from {session.loggedIn ? "member" : "guest"}</span><span>{session.archivedAt ? "Archived" : label(session.status)}</span></div>
          <div className="admin-visitor-chat-actions"><button disabled={busy.includes(session.id)} onClick={() => openVisitorChat(session)}>{busy === session.id ? "Opening…" : "Open chat"}</button><button className="archive" disabled={busy.includes(session.id)} onClick={() => moveVisitorChat(session, visitorChatView === "archive" ? "RESTORE" : "ARCHIVE")}>{busy.includes(session.id) && busy !== session.id ? "Moving…" : visitorChatView === "archive" ? "Move to inbox" : "Archive"}</button></div>
        </article>)}{!visitorChats.length && <p className="admin-empty">{visitorChatView === "archive" ? "No archived pop-up chats." : "No pop-up chats in Inbox."}</p>}</div>
        {visitorTranscript && <aside className="admin-transcript"><header><div><strong>{visitorTranscript.session.loggedIn ? visitorTranscript.session.memberName || "Logged-in member" : `Guest ${visitorTranscript.session.shortId}`}</strong><span>{visitorTranscript.session.pagePath}</span><span>{visitorTranscript.session.loggedIn ? "Logged in" : "Guest"}</span>{visitorTranscript.session.memberEmail && <a className="admin-visitor-email" href={`mailto:${visitorTranscript.session.memberEmail}`}>{visitorTranscript.session.memberEmail}</a>}{visitorTranscript.session.email && visitorTranscript.session.email !== visitorTranscript.session.memberEmail && <a className="admin-visitor-email" href={`mailto:${visitorTranscript.session.email}`}>{visitorTranscript.session.email}</a>}</div><button onClick={() => setVisitorTranscript(null)}>Close</button></header><div className="admin-transcript-audit"><strong>Access logged</strong><span>{visitorTranscript.session.accessedBy}</span></div><div className="admin-transcript-thread">{visitorTranscript.messages.map((message) => <article className={message.sender === "ADMIN" ? "creator" : "customer"} key={message.id}><strong>{message.sender === "ADMIN" ? "Administrator" : visitorTranscript.session.loggedIn ? visitorTranscript.session.memberName || "Logged-in member" : `Guest ${visitorTranscript.session.shortId}`}</strong><p>{message.body}</p><small>{new Date(message.createdAt).toLocaleString()}</small></article>)}</div><form className="admin-transcript-reply" onSubmit={sendVisitorReply}><textarea required maxLength={800} value={visitorReply} onChange={(event) => setVisitorReply(event.target.value)} placeholder="Reply to this pop-up chat…"/><button disabled={busy === `visitor-reply-${visitorTranscript.session.id}` || !visitorReply.trim()}>{busy === `visitor-reply-${visitorTranscript.session.id}` ? "Sending…" : "Send"}</button></form></aside>}</div>
      </>}
