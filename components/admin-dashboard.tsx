...
type AdminTab = "overview" | "profiles" | "accounts" | "conversations" | "assistants" | "payments" | "reports" | "audit";
...
type AdminConversation = { id: string; creatorName: string; creatorEmail?: string | null; creatorWallet?: string | null; customerName: string; customerEmail?: string | null; customerWallet?: string | null; messageCount: number; automatedCount: number; reviewNeededCount: number; reportCount: number; botEnabled: boolean; updatedAt: string };
type Transcript = { conversation: { id: string; creator: { name: string; email?: string | null }; customer: { name: string; email?: string | null }; accessReason: string; accessedBy: string }; messages: Array<{ id: string; senderRole: string; senderName: string; body: string; status: string; automated: boolean; automationRuleLabel?: string | null; automationMatched?: boolean | null; boostAmountUsdc: number; createdAt: string }> };
type BotRule = { id?: string; label: string; matchPhrases: string[]; response: string; priority: number; enabled: boolean };
type BotCreator = { creatorUserId: string; profileId: string; name: string; email?: string | null; enabled: boolean; disclosureLabel: string; fallbackResponse: string; rules: BotRule[] };
...
export function AdminDashboard() {
  ...
  const [botCreators, setBotCreators] = useState<BotCreator[]>([]);
  const [botEditor, setBotEditor] = useState<BotCreator | null>(null);
  ...
  async function loadBots() {
    ...
    if (response.status === 401) { setSignedIn(false); return; }
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Could not load automated assistants.");
    setBotCreators(data.creators || []);
    if (botEditor) setBotEditor((data.creators || []).find((creator: BotCreator) => creator.creatorUserId === botEditor.creatorUserId) || null);
  }
  async function loadDashboard() { const active = await loadProfiles(); if (active) await Promise.all([loadReports(), loadOperations(), loadConversations(), loadBots()]); }
  ...
  function editBotRule(index: number, changes: Partial<BotRule>) {
    setBotEditor((current) => current ? { ...current, rules: current.rules.map((rule, ruleIndex) => ruleIndex === index ? { ...rule, ...changes } : rule) } : current);
  }

  async function saveBot() {
    if (!botEditor) return;
    setBusy(botEditor.creatorUserId); setError("");
    const response = await fetch("/api/admin/bots", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(botEditor) });
    const data = await response.json();
    if (!response.ok) setError(data.error || "Automated assistant settings could not be saved.");
    else await loadBots();
    setBusy("");
  }
  ...
  return <main className="admin-shell admin-console">
    ...
    <div className="admin-console-grid"><aside><nav>{(["overview", "profiles", "accounts", "conversations", "assistants", "payments", "reports", "audit"] as AdminTab[]).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{label(item)}{item === "profiles" && pending.length ? <em>{pending.length}</em> : null}{item === "reports" && activeReports.length ? <em>{activeReports.length}</em> : null}</button>)}</nav></aside><section className="admin-console-content">{error && <div className="form-error">{error}</div>}
      ...
      {tab === "conversations" && <><div className="admin-page-heading"><span>AUDITED MESSAGE ACCESS</span><h2>Conversations</h2><p>Search conversation metadata first. Opening message content requires a reason and creates a permanent access record.</p></div><form className="admin-account-tools" onSubmit={(event) => { event.preventDefault(); loadConversations(conversationSearch).catch((caught) => setError(caught.message)); }}><input className="admin-search" value={conversationSearch} onChange={(event) => setConversationSearch(event.target.value)} placeholder="Search member, email, wallet or conversation ID"/><button className="admin-export-button" type="submit">Search conversations</button></form><div className="admin-conversation-layout"><div className="admin-conversation-list">{conversations.map((conversation) => <article key={conversation.id}><header><div><strong>{conversation.customerName}</strong><span>to {conversation.creatorName}</span></div><small>{new Date(conversation.updatedAt).toLocaleString()}</small></header><div><span>{conversation.messageCount} messages</span>{conversation.automatedCount > 0 && <span>{conversation.automatedCount} automated</span>}{conversation.reportCount > 0 && <span className="warning">{conversation.reportCount} reports</span>}{conversation.botEnabled && <span>Assistant on</span>}</div><p>{conversation.customerEmail || short(conversation.customerWallet)} → {conversation.creatorEmail || short(conversation.creatorWallet)}</p><button disabled={busy === conversation.id} onClick={() => openTranscript(conversation)}>{busy === conversation.id ? "Opening…" : "Open transcript with reason"}</button></article>)}{!conversations.length && <p className="admin-empty">No conversations found.</p>}</div>{transcript && <aside className="admin-transcript"><header><div><strong>{transcript.conversation.customer.name}</strong><span>with {transcript.conversation.creator.name}</span></div><button onClick={() => setTranscript(null)}>Close</button></header><div className="admin-transcript-audit"><strong>Audited access</strong><span>{transcript.conversation.accessedBy}</span><p>{transcript.conversation.accessReason}</p></div><div className="admin-transcript-thread">{transcript.messages.map((message) => <article className={message.senderRole === "CREATOR" ? "creator" : "customer"} key={message.id}>{message.automated && <span className="bot-message-label">Automated assistant</span>}<strong>{message.senderName}</strong><p>{message.body}</p><small>{new Date(message.createdAt).toLocaleString()} · {label(message.status)}</small></article>)}</div></aside>}</div></>}
      {tab === "assistants" && <><div className="admin-page-heading"><span>PRESET CREATOR ASSISTANTS</span><h2>Automated assistants</h2><p>Enable assistants only for selected creators. Replies use the preset library below and never call an external AI service.</p></div><div className="admin-bot-layout"><div className="admin-bot-list">{botCreators.map((creator) => <button className={botEditor?.creatorUserId === creator.creatorUserId ? "active" : ""} onClick={() => setBotEditor(structuredClone(creator))} key={creator.creatorUserId}><span><strong>{creator.name}</strong><small>{creator.email || "Wallet account"}</small></span><em>{creator.enabled ? "Enabled" : "Off"}</em></button>)}</div>{botEditor && <section className="admin-bot-editor"><header><div><span>CREATOR ASSISTANT</span><h3>{botEditor.name}</h3></div><label><input type="checkbox" checked={botEditor.enabled} onChange={(event) => setBotEditor({ ...botEditor, enabled: event.target.checked })}/> Enabled</label></header><label>Visible label<input value={botEditor.disclosureLabel} maxLength={80} onChange={(event) => setBotEditor({ ...botEditor, disclosureLabel: event.target.value })}/></label><label>Fallback for unmatched questions<textarea value={botEditor.fallbackResponse} maxLength={800} onChange={(event) => setBotEditor({ ...botEditor, fallbackResponse: event.target.value })}/></label><div className="admin-bot-rules">{botEditor.rules.map((rule, index) => <article key={rule.id || `${rule.label}-${index}`}><header><input value={rule.label} onChange={(event) => editBotRule(index, { label: event.target.value })}/><label><input type="checkbox" checked={rule.enabled} onChange={(event) => editBotRule(index, { enabled: event.target.checked })}/> On</label><button className="reject" onClick={() => setBotEditor({ ...botEditor, rules: botEditor.rules.filter((_, ruleIndex) => ruleIndex !== index) })}>Remove</button></header><label>Trigger phrases, comma separated<input value={rule.matchPhrases.join(", ")} onChange={(event) => editBotRule(index, { matchPhrases: event.target.value.split(",").map((phrase) => phrase.trim()).filter(Boolean) })}/></label><label>Response<textarea maxLength={800} value={rule.response} onChange={(event) => editBotRule(index, { response: event.target.value })}/></label><label>Priority<input type="number" min="0" max="10000" value={rule.priority} onChange={(event) => editBotRule(index, { priority: Number(event.target.value) })}/></label></article>)}</div><div className="review-actions"><button onClick={() => setBotEditor({ ...botEditor, rules: [...botEditor.rules, { label: "New response", matchPhrases: ["example"], response: "Thanks for your message.", priority: (botEditor.rules.at(-1)?.priority || 0) + 10, enabled: true }] })}>Add response</button><button disabled={busy === botEditor.creatorUserId} onClick={saveBot}>{busy === botEditor.creatorUserId ? "Saving…" : "Save assistant"}</button></div></section>}</div></>}
