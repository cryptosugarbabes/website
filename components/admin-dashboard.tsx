...
type VisitorChatSession = { id: string; shortId: string; status: string; pagePath: string; email?: string | null; archivedAt?: string | null; messageCount: number; visitorMessageCount: number; createdAt: string; lastSeenAt: string };
...
export function AdminDashboard() {
  ...
  async function moveVisitorChat(session: VisitorChatSession, action: "ARCHIVE" | "RESTORE") {
    ...
    if (!response.ok) setError(data.error || "Visitor chat could not be moved.");
    else {
  ...
  return <main className="admin-shell admin-console">
    ...
    <div className="admin-console-grid"><aside><nav>{(["overview", "profiles", "accounts", "visitor-chat", "conversations", "forums", "notifications", "payments", "reports", "audit"] as AdminTab[]).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{label(item)}{item === "profiles" && pending.length ? <em>{pending.length}</em> : null}{item === "visitor-chat" && visitorChatCounts.inbox ? <em>{visitorChatCounts.inbox > 99 ? "99+" : visitorChatCounts.inbox}</em> : null}{item === "reports" && activeReports.length ? <em>{activeReports.length}</em> : null}</button>)}</nav></aside><section className="admin-console-content">{error && <div className="form-error">{error}</div>}
      ...
      {tab === "visitor-chat" && <>
        <div className="admin-page-heading admin-visitor-chat-heading">
          ...
          <div><h2>Visitor chat</h2><nav aria-label="Visitor chat folders"><button className={visitorChatView === "inbox" ? "active" : ""} onClick={() => selectVisitorChatView("inbox")}>Inbox <em>{visitorChatCounts.inbox}</em></button><button className={visitorChatView === "archive" ? "active" : ""} onClick={() => selectVisitorChatView("archive")}>Open archives <em>{visitorChatCounts.archive}</em></button></nav></div>
          <p>Visitors can optionally share an email address. New visitor messages automatically return archived conversations to Inbox.</p>
        </div>
        <div className="admin-conversation-layout"><div className="admin-conversation-list">{visitorChats.map((session) => <article key={session.id}>
          <header><div><strong>Visitor {session.shortId}</strong><span>{session.pagePath}</span></div><small>{new Date(session.lastSeenAt).toLocaleString()}</small></header>
          {session.email && <a className="admin-visitor-email" href={`mailto:${session.email}`}>{session.email}</a>}
          <div><span>{session.messageCount} messages</span><span>{session.visitorMessageCount} from visitor</span><span>{session.archivedAt ? "Archived" : label(session.status)}</span></div>
