...
type ReviewProfile = { id: string; name: string; age: number; city: string; country: string; headline: string; bio: string; interests: string[]; status: string; rejectionReason?: string; photos: Array<{ id: string; url: string; approved: boolean }> };
...
export function AdminDashboard() {
  ...
  if (!signedIn) return <main className="admin-shell"><section className="admin-login"><span>CRYPTO SUGAR BABES</span><h1>Operations access</h1><p>Sign in on the main site with an approved administrator email, then return here. The emergency password remains available during the transition.</p><div className="review-actions"><a href="/">Email sign in</a></div><form onSubmit={login}><input type="password" required value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Emergency administrator password"/><button type="submit">Emergency sign in</button></form>{error && <div className="form-error">{error}</div>}</section></main>;
  ...
  return <main className="admin-shell admin-console">
    ...
    <div className="admin-console-grid"><aside><nav>{(["overview", "profiles", "accounts", "payments", "reports", "audit"] as AdminTab[]).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{label(item)}{item === "profiles" && pending.length ? <em>{pending.length}</em> : null}{item === "reports" && activeReports.length ? <em>{activeReports.length}</em> : null}</button>)}</nav></aside><section className="admin-console-content">{error && <div className="form-error">{error}</div>}
      ...
      {tab === "profiles" && <><div className="admin-page-heading"><span>CONTENT REVIEW</span><h2>Creator profiles</h2><p>Review profile copy and explicitly approve every photograph before public discovery.</p></div><section className="review-list">{profiles.length === 0 ? <p className="admin-empty">No profiles submitted.</p> : profiles.map((profile) => { const photosReady = profile.photos.length > 0 && profile.photos.every((photo) => photo.approved); return <article className="review-card" key={profile.id}><div className="review-photos">{profile.photos.length ? profile.photos.map((photo, index) => <div className="review-photo" key={photo.id}><img src={photo.url} alt={`${profile.name} photo ${index + 1}`}/><span className={photo.approved ? "approved" : "pending"}>{photo.approved ? "Approved" : "Needs review"}</span><div><button disabled={busy === photo.id || photo.approved} onClick={() => reviewPhoto(photo.id, "approve")}>Approve</button><button className="reject" disabled={busy === photo.id} onClick={() => reviewPhoto(photo.id, "reject")}>Reject</button></div></div>) : <div>No photos</div>}</div><div className="review-copy"><span className={`review-status status-${profile.status.toLowerCase()}`}>{label(profile.status)}</span><h2>{profile.name}, {profile.age}</h2><p className="location">{profile.city} · {profile.country}</p><h3>{profile.headline}</h3><p>{profile.bio}</p><div className="tag-row">{profile.interests.map((interest) => <span key={interest}>{interest}</span>)}</div>{profile.rejectionReason && <p className="rejection-note">Previous reason: {profile.rejectionReason}</p>}{!photosReady && <p className="review-guidance">Approve every photo before approving the public profile.</p>}<div className="review-actions"><button disabled={busy === profile.id || !photosReady} onClick={() => review(profile.id, "approve")}>Approve profile</button><button className="reject" disabled={busy === profile.id} onClick={() => review(profile.id, "reject")}>Reject with reason</button></div></div></article>; })}</section></>}
