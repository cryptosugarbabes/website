...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await query<MediaRow>(`
      SELECT m.storage_key, m.mime_type, m.is_approved, u.wallet_chain, u.wallet_address
    ...
    const ownsMedia = session?.chain === media.wallet_chain && session.address === media.wallet_address;
