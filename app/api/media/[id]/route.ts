...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await query<MediaRow>(`
      SELECT m.storage_key, m.mime_type, m.is_approved, u.wallet_chain, u.wallet_address, u.id AS user_id, u.status AS user_status
    ...
    if ((!media.is_approved || media.user_status !== "ACTIVE") && !ownsMedia) return new NextResponse(null, { status: 404 });
    const bytes = await fs.readFile(safeStoragePath(media.storage_key));
    return new NextResponse(bytes, {
      headers: {
        "content-type": media.mime_type,
        "content-length": String(bytes.length),
        "cache-control": media.is_approved && media.user_status === "ACTIVE" ? "public, max-age=86400, stale-while-revalidate=604800" : "private, no-store",
